...
inspection in Charmed OpenStack environments running OpenStack Yoga.
...
Placement is intentionally not packaged by this repository because the Yoga
Placement charm may not expose the same `policyd-override` configuration path.
Validate Placement policy override support for the target charm revision before
adding it.
...
The implied role is important because some Yoga policies check `role:reader`
...
Some Nova Yoga deployment-wide read APIs may still behave as project-scoped
policy checks. For those APIs, use an admin-project-scoped token combined with
the read-only `ai_observer` policy rules instead of assigning the user the
`admin` role.
...
- This baseline is focused on OpenStack Yoga and Charmed OpenStack.
...
- [Keystone Yoga default roles and scope model](https://docs.openstack.org/keystone/yoga/admin/service-api-protection.html)
- [Nova Yoga policy reference](https://docs.openstack.org/nova/yoga/configuration/policy.html)
- [Neutron Yoga policy reference](https://docs.openstack.org/neutron/yoga/configuration/policy.html)
- [Cinder Yoga policy reference](https://docs.openstack.org/cinder/yoga/configuration/block-storage/policy.html)
- [Glance Yoga policy source](https://opendev.org/openstack/glance/src/branch/stable/yoga/glance/common/policies)
- [Octavia Yoga policy source](https://opendev.org/openstack/octavia/src/branch/stable/yoga/octavia/policies)
