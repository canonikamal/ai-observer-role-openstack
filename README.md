# OpenStack Yoga AI Read-Only Role for Charmed OpenStack
...
- Allow broad visibility across Yoga services.
- Apply through Charmed OpenStack using Juju policy override resources.

Important notes:
- In Yoga, many services already use the reader role for read APIs.
...
- Some Yoga policies directly check role:reader instead of reusable reader rules.
...
for project in $(openstack project list -f value -c ID); do
  echo "$project"; openstack role add --user-domain default --user ai-agent --project "$project" ai_observer || true
done
```
...
- This baseline is Yoga-focused and targets core services in charmed deployments.
- Placement is not packaged here because the Yoga placement charm does not expose the same policyd-override configuration path.
...
- Keystone Yoga default roles and scope model
- Nova Yoga policy reference
- Neutron Yoga policy reference
- Cinder Yoga policy reference
- Glance Yoga policy reference
- Octavia Yoga policy reference
